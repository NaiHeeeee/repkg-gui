...
class WallpaperContextMenu {
    ...
    getText(textOrKey, fallback = '') {
        if (typeof textOrKey !== 'string') {
            return fallback;
        }

        if (window.i18n?.t && textOrKey.includes('.')) {
            const translated = window.i18n.t(textOrKey);
            if (translated !== textOrKey) {
                return translated;
            }
        }

        return textOrKey || fallback;
    }
    ...
    renderMenuItems() {
        ...
        this.menuItems.forEach((item) => {
            ...
            let iconHtml = '';
            if (item.icon) {
                iconHtml = `<span class="w-4 h-4">${item.icon}</span>`;
            }

            const text = this.getText(item.textKey, item.fallbackText);
            menuItem.innerHTML = `${iconHtml}<span>${text}</span>`;

            menuItem.addEventListener('click', (e) => {
                e.stopPropagation();
                if (item.callback) {
                    item.callback(this.currentWallpaper);
                }
                this.hideMenu();
            });
    ...
    // 添加菜单项的方法（供后续扩展使用）
    addMenuItem(textKey, callback, icon = '', fallbackText = '') {
        this.menuItems.push({
            type: 'item',
            textKey,
            callback,
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.wallpaperContextMenu.addMenuItem(
        'buttons.import_wallpaper_editor',
        (wallpaper) => {
            if (wallpaper) {
                // 设置当前选中的壁纸信息
